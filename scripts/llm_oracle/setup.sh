...
#
# The firewall allowlist has download.pytorch.org (torch CPU wheels),
# pypi.org, and files.pythonhosted.org (everything else). The oracle runs on
# the CPU: GPT-2 124M in fp32 takes seconds there, and the CPU wheel is a
# fraction of the size of the CUDA one.
set -euo pipefail
