...
- Current limitation: every test trips libc++ I/O false positives in Catch2's reporter startup (`std::ostream::__put_character_sequence`, locale-based float parsing, etc.). The MSAN-instrumented libc++ we build has gaps in its I/O state annotation. These reports are in compiled inline code, so `MSAN_OPTIONS=suppressions=...` cannot filter them. Practical use is therefore targeted single-test debugging when you suspect a specific uninitialized-read bug - ignore the Catch2/libc++ noise at the top of the trace and look at user-code frames. The full sweep is not currently a useful CI signal.
