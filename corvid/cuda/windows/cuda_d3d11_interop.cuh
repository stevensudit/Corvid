...
// surface load/store so a kernel can `surf2Dwrite` it. Reach its storage by
// mapping it per frame with a `cuda_d3d11_mapping`.
class cuda_d3d11_resource {
public:
#pragma region Construction

  explicit cuda_d3d11_resource(ID3D11Resource* resource,
      unsigned flags = cudaGraphicsRegisterFlagsSurfaceLoadStore) {
    cuda_last_status{
...
// Scoped map of a `cuda_d3d11_resource` for CUDA access; the unmap on scope
// exit also synchronizes CUDA work against the resource's subsequent D3D use.
// Create one per frame on the stack. The `cudaArray` it yields is borrowed,
