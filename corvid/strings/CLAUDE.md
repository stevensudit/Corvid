...
This folder holds the enum-free string utilities (the `strings` band). The
notes below cover the modern formatting path that lives here: thin
`std::formatter` specializations and format wrappers built on C++23
`std::format`. See [roadmap.md](roadmap.md) for the staged plan and rationale.
...
- Format wrappers COMPOSE the wrapped value; they do not inherit std types. We
  ...
  [string_view_wrapper.h](string_view_wrapper.h) composes rather
  than inherits.
