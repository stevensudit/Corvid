...
// Windows implementation of "os_mmap_file.h", over a `mapped_view`.
...
#pragma region Construction
  ...
  [[nodiscard]] static os_mmap_file map(const os_file& file) noexcept {
    return os_mmap_file{mapped_view::map_all(file)};
  }

#pragma endregion
#pragma region Accessors

  [[nodiscard]] bool is_mapped() const noexcept { return view_.is_mapped(); }
...
private:
  explicit os_mmap_file(mapped_view&& view) noexcept
      : view_{std::move(view)} {}
