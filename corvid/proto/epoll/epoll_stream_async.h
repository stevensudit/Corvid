...
//  `epoll_stream_conn` therefore invokes whichever handlers are currently
//  active without any knowledge of this class hierarchy.
//
...
protected:
  ...
  explicit epoll_stream_async_base(std::shared_ptr<epoll_stream_conn> conn)
      : conn_(std::move(conn)) {}
  ...
  bool install_handlers() {
    ...
    // than calling epoll_stream_conn private methods directly (lambdas do not
    ...
    return conn_->loop_.post([p = conn_] { return enable_reads(*p, false); });
  }

  // NOLINTBEGIN(bugprone-exception-escape)
  ~epoll_stream_async_base() {
    if (!conn_) return;
  ...
  // to `epoll_stream_conn` private members; lambdas defined inside member
  // functions do not inherit that access.

  // Set read-enabled state on `c` and propagate to the loop.
  static bool enable_reads(epoll_stream_conn& c, bool on = true) {
  ...
  static bool restore_reads(epoll_stream_conn& c) {
    c.recv_buf_.reads_enabled = !!c.own_handlers_.on_data;
    return c.refresh_read_interest();
  }

  // Clear `c`'s `recv_buf_` (`begin=end=0`) and return a
  // `epoll_recv_buffer_view` over it. The resume callback captures a
  // `shared_ptr<epoll_stream_conn>` to keep the connection alive while the
  // view is live. Called from `on_close` to satisfy any in-flight read
  // callback with an empty (EOF) view.
  static epoll_recv_buffer_view make_cleared_view_for(epoll_stream_conn& c) {
    c.recv_buf_.begin.store(0, std::memory_order::relaxed);
    c.recv_buf_.end.store(0, std::memory_order::relaxed);
    return epoll_recv_buffer_view{c.recv_buf_,
        [sp = c.self()](size_t n, size_t lse) { sp->resume_receive(n, lse); }};
  }

  // Wrap `epoll_stream_conn::exec_lambda` for derived classes. C++ friendship
  // is not inherited, so derived classes reach private `epoll_stream_conn`
  // members through these helpers.
  template<typename FN>
  bool exec_on_loop(FN&& fn) {
    return conn_->exec_lambda(execution::nonblocking, std::forward<FN>(fn));
  }

  // Wrap `epoll_stream_conn::enqueue_send`. Must be called on the loop thread.
...
// Usage:
//
//   epoll_stream_async_cb cb{conn_ptr.pointer()};  // or any
//   shared_ptr<epoll_stream_conn> cb.read([](epoll_recv_buffer_view v) {
...
//
// `EPOLLIN` is gated by `epoll_recv_buffer::reads_enabled`. `on_data` is set
...
private:
  ...
  bool invoke_read_cb(epoll_recv_buffer_view v) {
...
//
//   epoll_stream_async_coro coro{conn_ptr.pointer()};
