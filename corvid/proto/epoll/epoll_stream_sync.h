...
// `epoll_loop` or `epoll_stream_conn`.
//
...
// Typical usage:
//
//   auto conn = epoll_stream_sync::connect(ep, 5s);
//   assert(conn);
...
public:
  ...
  epoll_stream_sync() = default;

  epoll_stream_sync(epoll_stream_sync&&) noexcept = default;
  epoll_stream_sync& operator=(epoll_stream_sync&&) noexcept = default;
  ...
  // falsy `epoll_stream_sync` on connection failure.
  [[nodiscard]] static epoll_stream_sync
