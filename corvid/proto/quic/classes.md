...
- [http3_conn](http3_conn.h#L403) / [http3_conn_handlers](http3_conn.h#L182)
...
[quic_dgram_protocol](quic_dgram_plugins.h#L110) is the upper plugin. It
...
| ----- | ---- | ------------- |
| [quic_dgram_protocol&lt;QuicPlugin&gt;](quic_dgram_plugins.h#L110) | quic_dgram_plugins.h | Bundle template. Nests `router_plugin` and `session_plugin`; parameterized on the upper plugin (default `quic_no_op_plugin`). |
| `router_plugin` | [quic_dgram_plugins.h#L125](quic_dgram_plugins.h#L125) | Extracts the DCID as the routing key via `quic_version_cid`; turns long-header Initials into new server sessions. |
| `session_plugin` | [quic_dgram_plugins.h#L226](quic_dgram_plugins.h#L226) | Inherits `quic_session_io`. Owns `QuicPlugin plugin_` by value; installs it via `conn().set_handlers(&plugin_)`. Drives the read/expiry cycle and calls `plugin_.drain(now)`. |
...
| [quic_no_op_plugin](quic_dgram_plugins.h#L73) | quic_dgram_plugins.h | Inherits `quic_conn_handlers`. Holds `quic_session_io&`. The default/base upper plugin: its `drain` emits only non-stream frames (ACKs, MAX_DATA). |
...
| Class | File | Relationships |
| ----- | ---- | ------------- |
| [http3_conn](http3_conn.h#L403) | http3_conn.h | Wraps `nghttp3_conn` (HTTP/3 framing + QPACK). Forwards nghttp3's callback table into `http3_conn_handlers*`. Non-copyable, non-movable (nghttp3 stores `this`). Owned **by the upper plugin**, not by `quic_session_io`. |
| [http3_conn_handlers](http3_conn.h#L182) | http3_conn.h | Abstract base: HTTP/3 upcalls (`on_begin_headers`, `on_recv_header`, `on_end_headers`, trailers, `on_recv_data`, `on_send_data_ready`, `on_end_stream`, `on_h3_stream_close`, `on_recv_settings`, ...). Each per-stream upcall carries nghttp3's `stream_user_data`. |
| [http3_router](http3_plugins.h#L366) | http3_plugins.h | The upper plugin for HTTP/3. Inherits **both** `quic_conn_handlers` (transport upcalls in) **and** `http3_conn_handlers` (HTTP/3 upcalls out), owns an `http3_conn` by value (`h3_`), and holds a `quic_session_io&` (`io_`). Bridges mechanically in both directions and demuxes the connection-level HTTP/3 events to per-stream `http3_stream` objects it owns in `streams_` (`unordered_map<quic_stream_id, unique_ptr<http3_stream>>`), keyed by stream id. A server overrides `create_inbound_stream`; a client calls `add_stream` / `submit_request`. |
| [http3_server_router&lt;Stream&gt;](http3_plugins.h#L851) | http3_plugins.h | Ready-made server subclass of `http3_router`: overrides `create_inbound_stream` to mint a fixed `Stream` (must derive from `http3_stream`, be default-constructible) per peer-initiated request, so a server needs no router subclass of its own. Plug in as `quic_dgram_protocol<http3_server_router<Stream>>`. |
...
| [iov_queue&lt;Chunk, State&gt;](../iov_queue.h#L94) | ../iov_queue.h | Queue of owned byte buffers presented to a gather/scatter syscall as an `iovec` view; one queue serves either direction, carrying an optional per-queue `State`. Used by `quic_echo_plugin` as the send queue (with sticky `write_stream_flags`) and by `http3_stream` as both send and receive queues. |
| [quic_cid](quic_header.h#L142) / [quic_version_cid](quic_header.h#L204) | quic_header.h | Connection ID value type and the header decoder the `router_plugin` uses to recover the DCID. Also `quic_stream_id`, `quic_status`, and the stream/datagram flag enums. |
| [http3_headers](http3_header.h#L284) / [http3_field](http3_header.h#L268) | http3_header.h | Ordered collection of HTTP fields and its element (owned name / value plus `nv_flags` and `qpack_token`). Token-aware lookups (`find`, `find_unique`, `find_next`, `count`, `set_value`), a `stream_chunk` FIN marker, an implicit `span<const http3_field>` for the submit path. |
| [qpack_token](http3_header.h#L53) / [header_name](http3_header.h#L240) | http3_header.h | HTTP/3 header vocabulary: the QPACK well-known field-name tokens plus the compile-time-checked name views `header_name` and `header_name_and_enum` (the latter also carries the token) and `method_name`, with the `_header` / `_method` UDLs. Also the `nv_flags`, `stream_chunk`, and `http3_method` enums. |
