...
struct stream_conn_handlers {
  std::function<bool(stream_conn&, std::string&)> on_data = nullptr;
...
private:
  ...
  //
  // When `EPOLLHUP | EPOLLIN` fires in the same wakeup, `dispatch_event` calls
  // `on_readable()` first to drain any buffered data before calling
  ...
  [[nodiscard]] bool handle_readable() {
    ...
    if (!sock().recv(recv_buf_)) {
      ...
      return handle_read_eof() && false;
    }

    // If we read something, notify the user.
    if (!recv_buf_.empty()) return notify_read_ready();
    return true;
  }
