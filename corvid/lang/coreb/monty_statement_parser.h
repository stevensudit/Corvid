...
//   x = 5            ->  (define x 5)
//   f(x)             ->  (f x)
//   def inc(n):      ->  (define inc (lambda (n) (+ n 1)))
//     n + 1
...
//
//   program ::= { statement }
//   statement ::= define | def | if | return | reserve | expr NEWLINE
...
//
// where def/if/elif/else/return are contextual words recognized in
// statement-leading position, `=` desugars 1:1 to the kernel `define`,
// `:=` is rejected with a message reserving it for assignment, and blocks
// desugar to `(begin ...)` sequences except a `def` body, which splats
// into the lambda's implicit sequence.
//
...
// clause idiom. Any deeper `return` is an error, as is one outside `def`.
...
#pragma region builder
  ...
  struct builder {
    ...
    [[nodiscard]] result<stmt> parse_statement() {
      ...
      if (toks.at_word("def")) return parse_def();
    ...
    [[nodiscard]] result<stmt> parse_def() {
      toks.take(); // 'def'
      ...
      if (!toks.at(token_kind::rparen)) {
        for (;;) {
          ...
          params.push_back(value{rt.intern(toks.take().text)});
    ...
    // Check a `return`'s placement: inside `def` only, and only where the
    // rewrite can express it.
    [[nodiscard]] std::optional<source_error>
    check_return(const ret_stmt& r, bool in_def, bool expressible) const {
      if (!in_def)
    ...
    // rewrite.
    //
    // `in_def` marks a body somewhere inside a function; `tail` marks one
    ...
    desugar_body(std::span<const stmt> stmts, bool in_def, bool tail) {
      ...
      for (size_t ndx = 0; ndx < stmts.size(); ++ndx) {
        ...
        if (const auto* r = std::get_if<ret_stmt>(&s.node)) {
          if (auto e = check_return(*r, in_def, tail && last))
            return std::move(*e);
        ...
        if (tail && !last && !f.has_else && all_arms_return(f)) {
          auto rest = desugar_body(stmts.subspan(ndx + 1), in_def, tail);
          if (!rest) return rest;
          auto guard = desugar_if(f, in_def, true, begin_of(*rest));
          if (!guard) return guard;
          out.push_back(*guard);
          return out;
        }
        auto form = desugar_if(f, in_def, tail && last, std::nullopt);
        if (!form) return form;
        out.push_back(*form);
      }
      return out;
    ...
    [[nodiscard]] result<value> desugar_if(const if_stmt& f, bool in_def,
        bool tail, std::optional<value> forced_else) {
      std::optional<value> chain = std::move(forced_else);
      if (f.has_else) {
        auto body = desugar_body(f.else_body, in_def, tail);
        if (!body) return body;

        chain = begin_of(*body);
      }
      for (const auto& arm : std::views::reverse(f.arms)) {
        auto body = desugar_body(arm.body, in_def, tail);
        if (!body) return body;
