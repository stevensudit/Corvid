...
- To carry "no value", choose by how easy the missing-value path is to forget:
  - A natural sentinel that is never valid in context (`nullptr`, empty string,
    ...
    `find_opt`).
  - Reach for `std::optional`, or `value_or_error` when failure must carry
    *why*, when forgetting the check is plausible. (`value_or_error`, in
    "corvid/containers/core/value_or_error.h", is the library's deliberately
    reduced take on `std::expected`; use it rather than `std::expected`.)
    Both are options; **neither is mandated.** This is not Go, and even
    `optional` is often excessive.
