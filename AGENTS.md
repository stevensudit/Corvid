...
- Before reporting test failures, prefer one of these workflows:
  - Run `./cleanbuild.sh` from the repo root when you want the same clean rebuild path used in Codex.
  - For fast iteration on a single test or file, use `cleanbuild.sh` while specifying the target cpp.
