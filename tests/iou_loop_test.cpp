...
void IouLoop_NopCompletion() {
  ...
  if (true) {
    ...
    const auto token = runner->submit_nop(
        [&](completion_id, iou_res res, iou_cqe_flags flags) {
...
void IouLoop_MultipleNops() {
  ...
  if (true) {
    ...
    for (int i = 0; i < 4; ++i) {
      submitted =
          submitted &&
          runner->submit_nop([&](completion_id, iou_res, iou_cqe_flags) {
...
void IouLoop_RecvWriteFixed() {
  ...
  if (true) {
    ...
    const auto recv_token = runner->submit_read_buffer(recv_sock,
