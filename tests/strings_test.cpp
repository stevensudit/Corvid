...
TEST_CASE("StringUtilsTest_LocateEdges", "[StringUtilsTest]") {
  ...
  if (true) {
    ...
    CHECK((strings::rlocate<npos_choice::size>(s, {'y', 'z'})) ==
          ((location{s.size(), 2})));
    CHECK(
        (strings::locate(s, {"uvw"sv, "xyz"sv})) == ((location{npos, npos})));
    CHECK((strings::locate<npos_choice::size>(s, {"uvw"sv, "xyz"sv})) ==
          ((location{s.size(), 2})));
    CHECK(
        (strings::rlocate(s, {"uvw"sv, "xyz"sv})) == ((location{npos, npos})));
...
TEST_CASE("StringUtilsTest_Substitute", "[StringUtilsTest]") {
  ...
  if (true) {
    ...
    CHECK((strings::substitute(s, strings::as_views(f),
              strings::as_views(t))) == (4U));
    CHECK((s) == ("cdabefghijcdabefghij"));
...
TEST_CASE("StringUtilsTest_AppendNum", "[StringUtilsTest]") {
  if (true) {
    ...
    CHECK(
        ((strings::num_as_string<16>(uint64_t(0)))) == ("0x0000000000000000"));
    ...
    CHECK(((strings::num_as_string<std::chars_format::hex>(double(0.25)))) ==
          ("1p-2"));
    CHECK(((strings::num_as_string<std::chars_format::fixed>(
