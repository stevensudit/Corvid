...
void LangTest_Flattening() {
  ...
  EXPECT_EQ((root->print()), "and:(exists:(A), and:(exists:(B), exists:(C)))");
  root = dnf::convert(root);
...
void LangTest_Distribution() {
  ...
  root = M<and_junction>(M<exists>("A"s),
      M<or_junction>(M<exists>("B"s), M<exists>("C"s)));
  EXPECT_EQ((root->print()), "and:(exists:(A), or:(exists:(B), exists:(C)))");
  root = dnf::convert(root);
  ...
  root = M<or_junction>(M<exists>("A"s),
      M<and_junction>(M<exists>("B"s), M<exists>("C"s)));
  EXPECT_EQ((root->print()), "or:(exists:(A), and:(exists:(B), exists:(C)))");
  root = dnf::convert(root);
  EXPECT_EQ((root->print()), "or:(exists:(A), and:(exists:(B), exists:(C)))");
...
void LangTest_Comprehensive() {
  ...
  root = M<and_junction>(M<or_junction>(M<exists>("A"s), M<exists>("B"s)),
      M<exists>("C"s));
  EXPECT_EQ((root->print()), "and:(or:(exists:(A), exists:(B)), exists:(C))");
  root = dnf::convert(root);
  EXPECT_EQ((root->print()),
  ...
  root = M<and_junction>(M<and_junction>(M<exists>("A"s), M<exists>("B"s)),
      M<exists>("C"s));
  EXPECT_EQ((root->print()), "and:(and:(exists:(A), exists:(B)), exists:(C))");
  root = dnf::convert(root);
  EXPECT_EQ((root->print()), "and:(exists:(A), exists:(B), exists:(C))");
  ...
  root = M<or_junction>(M<exists>("D"s),
      M<or_junction>(M<exists>("E"s), M<exists>("F"s)));
  EXPECT_EQ((root->print()), "or:(exists:(D), or:(exists:(E), exists:(F)))");
  root = dnf::convert(root);
...
MAKE_TEST_LIST(LangTest_Constants, LangTest_Negation, LangTest_Flattening,
    LangTest_Distribution, LangTest_Wikipedia, LangTest_Comprehensive);
