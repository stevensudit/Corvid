...
TEST_CASE("quic_connection_id basics", "[quic]") {
  SECTION("value-initialized is empty") {
    const quic::quic_connection_id cid{};
    CHECK(cid.length() == 0);
    CHECK(cid.empty());
  }
  ...
  SECTION("byte-span round-trip") {
    ...
    const quic::quic_connection_id cid{bytes};
  ...
  SECTION("equal CIDs hash equally") {
    ...
    const quic::quic_connection_id a{bytes};
    const quic::quic_connection_id b{bytes};
    CHECK(a == b);
    CHECK(std::hash<quic::quic_connection_id>{}(a) ==
          std::hash<quic::quic_connection_id>{}(b));
  }

  SECTION("different lengths compare unequal") {
    const std::array<uint8_t, 8> a_bytes{
    ...
    const quic::quic_connection_id a{a_bytes};
    const quic::quic_connection_id b{b_bytes};
    CHECK(a != b);
    CHECK(a < b);
  }
  ...
  SECTION("ngtcp2_cid round-trip") {
    ...
    const quic::quic_connection_id cid{in};
    REQUIRE(cid.length() == bytes.size());
    CHECK(std::ranges::equal(cid.bytes(), bytes));
    ...
    CHECK(std::ranges::equal(
        std::span<const uint8_t>{out.data, out.datalen}, bytes));
  }
}
